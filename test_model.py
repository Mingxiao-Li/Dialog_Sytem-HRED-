...
def inference(hparams, model, vocab, data, chat_mode = False):
    ...
    if not chat_mode:
        ...
        if even:
            logger.info("Target User 1: {}".format(" ".join([vocab["itow"].get(w.item(),"<unk>") for w in data["tgt"][0]])))
        else:
            logger.info("Target User 2: {}".format(" ".join([vocab["itow"].get(w.item(), "<unk>") for w in data["tgt"][0]])))
    with torch.no_grad():
